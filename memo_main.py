...
def testlist():
    ...
    questions_listmodel.form_list.append(frm)
    frm = Question('Скупий', 'mean', 'man', 'skupi', 'maen')
    questions_listmodel.form_list.append(frm)
    frm = Question('білий', 'white', 'bele', 'whait', 'beli')
    questions_listmodel.form_list.append(frm)
    frm = Question('Чорний', 'black', 'snikers', 'bleak', 'min')
    questions_listmodel.form_list.append(frm)
...
def add_form():
    ...
    index = questions_listmodel.index(
        last)  # отримуємо об'єкт класу QModelIndex, який відповідає останньому елементу в даних
    ...
    txt_Question.setFocus(
        Qt.TabFocusReason)  # переводимо фокус на поле редагування питання, щоб одразу прибиралися "болванки"
